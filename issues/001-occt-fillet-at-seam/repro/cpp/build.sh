...
OCCT_INC="${OCCT_INC:-C:/dev/freecad-kernel-fixes/build/occt-inc}"
IMPLIB="${IMPLIB:-C:/dev/freecad-kernel-fixes/build/implib}"
OUTDIR="${OUTDIR:-C:/dev/freecad-kernel-fixes/build}"
...
"$VS/bin/Hostx64/x64/cl.exe" /nologo /std:c++17 /EHsc /MD /O2 /D_USE_MATH_DEFINES \
  /I"$OCCT_INC" \
  "C:/dev/freecad-kernel-fixes/issues/001-occt-fillet-at-seam/repro/cpp/seam_fillet_repro.cpp" \
