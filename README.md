...
| [008](issues/008-fillet-rim-ending-on-a-face) | Скругление кромки выреза, обрывающейся о грань, отказывает при r ≥ 0.25; замкнутая кромка скругляется | не определено, вероятно OCCT | измерено; обход кольцом измерен |
| [009](issues/009-occt-bop-periodic-pcurve) | Булева операция молча теряет инструмент, если грань на периодической поверхности шире периода на 1e-5 (резьба из STEP): сдвиг на период учитывается дважды | OCCT, `BOPTools_AlgoTools2D` | **исправлено** — патч в одну строку, проверен в OCCT и внутри копии FreeCAD, есть установка; не установлен |
| [010](issues/010-display-mesh-long-bspline-thread) | Открытие сборки 55 с вместо 2.5 с: сетка отображения ШВП из STEP (10 B-сплайновых граней по 1305 полюсов) при сохранённом Angular Deflection 6.4° — 1 008 762 треугольника, одна грань 46 с | FreeCAD, `ViewProviderPartExt` (выбор параметров) + OCCT `BRepMesh` (стоимость `ControlSurfaceDeflection`) | измерено; обход (28.5°) и экспериментальный `ControlSurfaceDeflection=false` проверены внутри копии FreeCAD; FreeCAD не пересобран |
